function [twix_obj,fid] = me_mapVBVD(filename,varargin)
...
%
% Mark Elliott, 15.12.21    added arg.verbose to turn off verbiage and progress bar
%                           added return TWIX filepointer so it can be CLOSED!!
%                           added parses wipmem params for labels in unused longs
...
while k <= numel(varargin)
    ...
    switch lower(varargin{k})
        ...
        case {'rawdatacorrect','dorawdatacorrect'}
         ...
         case {'verbose'} % % MELLIOTT
            if numel(varargin) > k && ~ischar(varargin{k+1})
                arg.verbose = logical(varargin{k+1});
                k = k+2;
            else
                arg.verbose = true;
                k = k+1;
...
if and(firstInt < 10000, secondInt <= 64)
    version = 'vd';
    if (arg.verbose), disp('Software version: VD (!?)'); end
    ...
    version  = 'vb';
    if (arg.verbose), disp('Software version: VB (!?)'); end
    measOffset{1} = 0;
...
if (strcmp(version, 'vb')) % not implemented/tested for vd, yet
    ...
    if (arg.verbose), disp('Read raw data correction factors'); end
end
...
for s=1:NScans
    ...
    rstraj = [];
    if arg.bReadHeader
        [twix_obj{s}.hdr, rstraj] = me_read_twix_hdr(fid);
    end

    % declare data objects:
    twix_obj{s}.image         = me_twix_map_obj(arg,'image',filename,version,rstraj);
    twix_obj{s}.noise         = me_twix_map_obj(arg,'noise',filename,version);
    twix_obj{s}.phasecor      = me_twix_map_obj(arg,'phasecor',filename,version,rstraj);
    twix_obj{s}.phasestab     = me_twix_map_obj(arg,'phasestab',filename,version,rstraj);
    twix_obj{s}.phasestabRef0 = me_twix_map_obj(arg,'phasestab_ref0',filename,version,rstraj);
    twix_obj{s}.phasestabRef1 = me_twix_map_obj(arg,'phasestab_ref1',filename,version,rstraj);
    twix_obj{s}.refscan       = me_twix_map_obj(arg,'refscan',filename,version,rstraj);
    twix_obj{s}.refscanPC     = me_twix_map_obj(arg,'refscan_phasecor',filename,version,rstraj);
    twix_obj{s}.refscanPS     = me_twix_map_obj(arg,'refscan_phasestab',filename,version,rstraj);
    twix_obj{s}.refscanPSRef0 = me_twix_map_obj(arg,'refscan_phasestab_ref0',filename,version,rstraj);
    twix_obj{s}.refscanPSRef1 = me_twix_map_obj(arg,'refscan_phasestab_ref1',filename,version,rstraj);
    twix_obj{s}.RTfeedback    = me_twix_map_obj(arg,'rtfeedback',filename,version,rstraj);
    twix_obj{s}.vop           = me_twix_map_obj(arg,'vop',filename,version); % tx-array rf pulses
    
    % print reader version information
    if s==1
        if (arg.verbose), fprintf('Reader version: %d', twix_obj{s}.image.readerVersion); end
        ...
        if (arg.verbose), fprintf(' (UTC: %s)\n', datestr(date_str));  end %Jamie Near - pass date_str as string for 
    ...
    if (arg.verbose), fprintf('Scan %d/%d, read all mdhs:\n', s, NScans ); end

    [mdh_blob, filePos, isEOF] = loop_mdh_read( fid, version, NScans, s, measOffset{s}, measLength{s}, arg);  % uint8; size: [ byteMDH  Nmeas ]
...
% ----------------------------------------------------------------------------------------------
% --- Pad WipMem params and replace empty cells w/ NaNs so we know they were missing --- MELLIOTT
if (iscell(twix_obj)), t = twix_obj{end};
else,                  t = twix_obj; end
if (isfield(t.hdr,'Phoenix') && isfield(t.hdr.Phoenix,'sWipMemBlock') && isfield(t.hdr.Phoenix.sWipMemBlock,'alFree'))
    if (iscell(twix_obj))
        lvals = twix_obj{end}.hdr.Phoenix.sWipMemBlock.alFree;
        dvals = twix_obj{end}.hdr.Phoenix.sWipMemBlock.adFree;
    else
        lvals = twix_obj.hdr.Phoenix.sWipMemBlock.alFree;
        dvals = twix_obj.hdr.Phoenix.sWipMemBlock.adFree;
    end

    max_doubles = 16;
    if (numel(dvals) < max_doubles)
        filler = cell(1,max_doubles-numel(dvals));
        dvals  = [dvals filler];
    end
    dvals(cellfun(@(x) isempty(x), dvals)) = {NaN};

    max_longs = 64;
    if (numel(lvals) < max_doubles)
        filler = cell(1,max_doubles-numel(lvals));
        lvals  = [lvals filler];
    end
    lvals(cellfun(@(x) isempty(x), lvals)) = {NaN};

    wipmem_struct = dicom_get_wipmemlabels(lvals,dvals);  % create a labeled struct of wipmem params

    if (iscell(twix_obj))
        twix_obj{end}.hdr.Phoenix.sWipMemBlock.adFree = dvals;
        twix_obj{end}.hdr.Phoenix.sWipMemBlock.alFree = lvals;
        twix_obj{end}.hdr.Phoenix.sWipMemBlock.wipmem_struct = wipmem_struct;
    else
        twix_obj.hdr.Phoenix.sWipMemBlock.adFree = dvals;
        twix_obj.hdr.Phoenix.sWipMemBlock.alFree = lvals;
        twix_obj.hdr.Phoenix.sWipMemBlock.wipmem_struct = wipmem_struct;
    end
end
% ----------------------------------------------------------------------------------------------
...
function [mdh_blob, filePos, isEOF] = loop_mdh_read( fid, version, Nscans, scan, measOffset, measLength, arg)
...
% http://undocumentedmatlab.com/blog/explicit-multi-threading-in-matlab-part1
    ...
    while true
        ...
        if progress > last_progress  + 0.01
            ...
            if (arg.verbose), waitbar(progress, h, progress_str); end % MELLIOTT
        end

        cPos = cPos + ulDMALength;
    end % while true
    if (arg.verbose), delete(h); end % MELLIOTT
    ...
    if (arg.verbose), fprintf('%8.1f MB read in %4.0f s\n', measLength/1024^2, round(toc(t0))); end

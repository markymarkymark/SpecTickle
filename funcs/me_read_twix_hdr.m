...
%
% MELLIOTT - modified to keep 'swipmemblock.alfree' params as int32, rather than convert to double
...
% MELLIOTT - catch that LONG WIPparams need to be 'int32'!!! not floats
if (contains(lower(var.name),'swipmemblock.alfree'))
    value = textscan(var.value,'%d32');
    value = value{1};
end
...
% if (contains(lower(var.name),'swipmemblock.adfree'))
%     fprintf(2,'name = %s\n',var.name);
%     xx=5;
% end
